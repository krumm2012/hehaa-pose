...
from font_manager import get_font_manager  # 导入字体管理器
...
def process_video(video_path, config):
    ...
    # 初始化字体管理器
    font_manager = get_font_manager(config)
    ...
    while True:
        ...
        if person_keypoints_list and len(person_keypoints_list) > 0:
            ...
            if all(keypoints.get(kp) for kp in ["right_shoulder", "right_elbow", "right_wrist"]):
                ...
                if keypoints["right_elbow"]:
                    ...
                    display_frame = font_manager.put_text_with_font(
                        display_frame, 
                        f"{right_elbow_angle:.0f}°", 
        ...
        if active_ball_this_frame_list:  # 检查列表是否非空
            ...
            display_frame = font_manager.put_text_with_font(
                display_frame, 
                f"球位置: ({int(ball_pos[0])}, {int(ball_pos[1])})", 
                (text_x_offset, current_y),
                font_scale=0.7, 
                color=(255, 255, 255), 
                thickness=2
            )
        ...
        # 显示挥拍阶段估计
        phase_est = full_analysis_metrics.get("phase_estimation", "未知")
        display_frame = font_manager.put_text_with_font(
            display_frame, 
            f"估计阶段: {phase_est}", 
            (text_x_offset, current_y), 
