...
class RealtimeSwingOutputManagerTests(unittest.TestCase):
    ...
    def test_publishes_json_html_and_async_event_clip(self):
        ...
        self.assertIn('aria-label="页面刷新控制"', html)
        self.assertIn('id="refresh-start"', html)
        self.assertIn('id="refresh-stop"', html)
        self.assertIn("sessionStorage.setItem(refreshStorageKey", html)
        self.assertIn("if (!autoRefreshEnabled || coachFeedPending) return", html)
        self.assertIn("setInterval(refreshPreview, 1000)", html)
        self.assertIn("if (!autoRefreshEnabled) return", html)
        ...
        self.assertIn("manualReviewRequestHeaders", html)
        self.assertIn("headers['X-Control-Token']", html)
        self.assertIn("headers['X-Manual-Review-Report']", html)
        self.assertIn("headers: await manualReviewRequestHeaders()", html)
